 ...
 * @param {function} next
 */
module.exports = function datRoutesInit(server, opts, next) {
  /** @type {SwarmManager} */
  const swarmManager = server.swarmManager;

  const validateCollDirRequest = async body => {
    if (!body) {
    ...
    server.log.info(swarmManager.actualDirPath(body.collDir));
