...
class SwarmManager extends EventEmitter {
  ...
  _doSyncRemoval(toSync) {
    ...
    for (let i = 0; i < toSync.length; ++i) {
      keep.add(this._actualDirPath(toSync[i]))
    }
  ...
  async initDat(directory) {
    ...
    const { discoveryKey, datKey } = await this._initDat(actualPath)
...
function swarmManagerPlugin(fastify, opts, next) {
  debug('Initializing')
  const swarmMan = new SwarmManager(opts)
  fastify
