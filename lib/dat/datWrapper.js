...
class DatWrapper {
  ...
  static from(datOrDirOrStorage, opts) {
    if (datOrDirOrStorage instanceof Dat) {
      return Promise.resolve(new DatWrapper(datOrDirOrStorage))
    }
    if (!opts) opts = {}
  ...
  async jsonCreateOrUpdate(data) {
    ...
    // does exist, only update it if data is non-null
    if (data != null) {
      const existing = await this.jsonRead()
      return this.jsonWrite({ ...existing, ...data })
    }
  }
